...
import { resolveBundledConsentVersion } from '../consent/default-consent-template';
import {
  Firestore,
  collection,
  doc,
...
export class ConsentTemplateVersionsService extends FirestoreBaseRepository<ConsentTemplateVersion> {
  ...
  override get(id: string) {
    return super.get(id).pipe(map((version) => version ?? resolveBundledConsentVersion(id)));
  }
  ...
  async createDraft(
    ...
    const nextVersionNumber = Math.max(0, ...existing.docs.map((version) =>
      Number(version.data()['versionNumber']) || 0,
    )) + 1;
    return this.create({
      templateId,
